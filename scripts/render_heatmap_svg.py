...
""".strip()
    ...
    parts = [
        ...
        f'<style>\n{css}\n'
        f'@keyframes loopHeatmap {{\n'
        f'  0%, 40% {{ opacity: 1; }}\n'
        f'  45%, 95% {{ opacity: 0; }}\n'
        f'  100% {{ opacity: 1; }}\n'
        f'}}\n'
        f'@keyframes loopGame {{\n'
        f'  0%, 40% {{ opacity: 0; }}\n'
        f'  45%, 95% {{ opacity: 1; }}\n'
        f'  100% {{ opacity: 0; }}\n'
        f'}}\n'
        f'#heatmap-view {{ animation: loopHeatmap 14s ease-in-out infinite; }}\n'
        f'#bomberman-view {{ animation: loopGame 14s ease-in-out infinite; opacity: 0; }}\n'
        f'</style>',
        '<defs>'
    ...
    for ci, label in month_labels:
        x = grid_left + ci * STEP
        parts.append(f'<text x="{x}" y="{TITLEBAR_H + 14}" fill="var(--text-muted)" font-size="10">{label}</text>')
